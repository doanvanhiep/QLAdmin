...
export class GiangvienComponent implements OnInit {
  ...
  get f() {
    return this.giangvienForm.controls;
  }
  getListGiangVien() {
  ...
  sua() {
    ...
    if (idImg === this.giangvienByID.HinhAnh) {
      this.giangvienService
        .suaGiangVien(
          this.giangvienByID.IDGiangVien,
          this.f.HoTen.value,
          this.f.DiaChi.value,
          this.f.SoDienThoai.value,
          this.f.Email.value,
  ...
  getGiangVienByID(idGiangVien) {
    this.giangvienByID = this.listGiangVien.filter(
      (item) => item.IDGiangVien === +idGiangVien
    )[0];
  }
