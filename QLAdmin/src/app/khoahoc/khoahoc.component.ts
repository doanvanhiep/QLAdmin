...
export class KhoahocComponent implements OnInit {
  ...
  get f() {
    return this.khoahocForm.controls;
  }
  them() {
    this.khoahocService
      .themKhoaHoc(this.f.TenKhoaHoc.value, this.f.GhiChu.value)
      .pipe()
      .subscribe((res) => {
  ...
  sua() {
    this.khoahocService
      .suaKhoaHoc(
        this.khoahocByID.IDKhoaHoc,
        this.f.TenKhoaHoc.value,
        this.f.GhiChu.value
      )
      .pipe()
      .subscribe((res) => {
  ...
  getKhoaHocByID(idKhoaHoc) {
    this.khoahocByID = this.listKhoaHoc.filter(
      (item) => item.IDKhoaHoc == +idKhoaHoc
    )[0];
  }

  shareData(IDKhoaHoc) {
    this.share.shareDataKhoaHoc(
      IDKhoaHoc,
      this.listKhoaHoc.filter((item) => item.IDKhoaHoc === IDKhoaHoc)[0]
    );
  }
