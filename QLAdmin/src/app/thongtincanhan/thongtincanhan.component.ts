...
export class ThongtincanhanComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.isGiangVien) this.serviceSelect = this.giangvienService;
    else this.serviceSelect = this.quantrivienService;
  ...
  createForm(thongtincanhan) {
    ...
    let img = this.thongtincanhan
      ? thongtincanhan.HinhAnh
      : "1Pgd3rbBMeKd9TKCBbf5C2hn005-cbD1c";
  ...
  getThongTin() {
    this.serviceSelect
      .getThongTinByTenTaiKhoan(this.TenTaiKhoan)
      .pipe()
      .subscribe((res) => {
        this.thongtincanhan = res.result.data[0];
        this.createForm(this.thongtincanhan);
        this.imgURL =
          "https://drive.google.com/uc?id=" + this.thongtincanhan.HinhAnh;
      });
  }
  ...
  get f() {
    return this.thongtincanhanForm.controls;
  }
  CapNhap() {
    if (this.newIDFile == -1) {
    ...
    this.serviceSelect
      .suaThongTinCaNhan(
        this.thongtincanhan.TenTaiKhoan,
        this.f.HoTen.value,
        this.f.DiaChi.value,
  ...
  get fDoiMatKhau() {
    return this.doimatkhauForm.controls;
  }
  doiMK() {
    let resCondition = this.checkConditionPassword(
      this.fDoiMatKhau.MatKhauCu.value,
      this.fDoiMatKhau.MatKhauMoi.value,
      this.fDoiMatKhau.MatKhauMoiXacNhan.value
    );
    if (!resCondition) {
      return;
    }
  ...
  checkConditionPassword(MatKhauCu, MatKhauMoi, MatKhauMoiXacNhan) {
    if (MatKhauCu == "") {
      this.toast.error("Vui lòng nhập mật khẩu cũ!", "Thông báo");
      return false;
    }
    if (MatKhauMoi == "") {
      this.toast.error("Vui lòng nhập mật khẩu mới!", "Thông báo");
      return false;
    }
    if (MatKhauMoiXacNhan == "") {
      this.toast.error("Vui lòng nhập xác nhận mật khẩu mới!", "Thông báo");
      return false;
    }
    if (MatKhauMoi.length < 8) {
      this.toast.error(
        "Vui lòng nhập mật khẩu mới có chiều dài lớn hơn 8!",
        "Thông báo"
      );
      return false;
    }
